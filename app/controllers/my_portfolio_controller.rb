class MyPortfolioController < ApplicationController
  ...
  before_action :set_stock, only: %i[sell]
  ...
  def sell
    sold_stock = current_user.stock_purchases.find_by(stock_id: params[:id])
    ...
    if sold_stock
      ...
      redirect_to home_myportfolio_path, alert: 'Unable to find stock to sell.'
    end
  end
